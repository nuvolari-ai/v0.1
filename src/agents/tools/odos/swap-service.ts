...
export class TokenSwapService {
  ...
  _replaceNativewWithZero(tokenOutId: string) {
    if (getAddress(tokenOutId) === getAddress('0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE')) {
  ...
  async createSwapPayloadFromInsight(
    ...
    const quoteRequest: PathRequestV2 = {
      chainId,
      inputTokens: [{
        tokenAddress: this._replaceNativewWithZero(insight.tokenInId),
        amount: parsedAmount
      }],
      outputTokens: [{
        tokenAddress: this._replaceNativewWithZero(insight.tokenOutId),
        proportion: 1.0
      }],
